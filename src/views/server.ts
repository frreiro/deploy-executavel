...
const start = (PORT: number, API_URL: string) => {
    ...
    const relativeViewFolder = path.join(path.dirname(__dirname), currentFolder, 'public')
    ...
    const server = app.listen(PORT || 4000, () => console.log(`Server is running at ${PORT}`))
